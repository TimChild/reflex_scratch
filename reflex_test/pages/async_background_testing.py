    ...
    return rx.container(
        rx.vstack(
            ...
            rx.card(
                ...
                rx.button("Change color",
                          on_click=ScratchState.change_color_by_method),
                background_color=rx.color(ScratchState.color, alpha=True),
            ),
            rx.card(
                rx.heading("Update state async background task"),
                rx.button("Change color",
                          on_click=ScratchState.async_change_color),
                background_color=rx.color(ScratchState.color, alpha=True),
            ),
            rx.card(
                rx.heading("Update state async background task method call"),
                rx.button("Change color",
                          on_click=ScratchState.async_change_color_by_method),
                background_color=rx.color(ScratchState.color, alpha=True),
            ),
            rx.card(
                rx.heading("Update other state directly"),
                rx.button("Change color",
                          on_click=ScratchState.directly_change_other_state),
            ),
            rx.box(
                width="50em",
                height="10em",
            ...
            rx.button(rx.text(ScratchState.number.to(str)),
                      on_click=ScratchState.start_several_tasks_via_yields),
            width="100%",
        ),
    )
