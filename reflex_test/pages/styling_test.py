    ...
    return rx.box(
        rx.box(
            ...
            style=rx.style.Style(
                style_dict={"background_color": "darkorchid", rx.style.media_query(3): {"background_color": "green"}}
            ),
        ),
        rx.container(
            rx.heading("Box that should show smaller box on hover"),
            rx.box(
                rx.text("select by id"),
                position="absolute",
                top="10px",
                right=0,
                height="50%",
            ...
            rx.box(
                rx.text("select by element"),
                rx.text("Also affects heading"),
                position="absolute",
                top="10px",
                right="100px",
                height="50%",
            ...
            style=rx.style.Style(
                style_dict={
                    "&:hover": {
                        "#inner-box": {"background_color": "green", "transition": "1s"},
                        ".rt-Box": {
