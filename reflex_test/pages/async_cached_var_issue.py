...
def index() -> rx.Component:
    return rx.container(
        rx.card(
            ...
            rx.input(value=HandlerState.input_a, label="Input A",
                     on_change=HandlerState.set_input_a),
        ...
        rx.hstack(
            rx.card(
                ...
                rx.markdown(
                    f"{DisplayState.data_a_cached}\n\n{DisplayState.data_b_cached}"),
            ),
            rx.card(
                rx.heading("Alternative Display of data", size="5"),
                rx.markdown(
                    f"{AlternativeDisplayState.data_attr_a}\n\n{AlternativeDisplayState.data_attr_b}"),
                rx.button("Update Display Info",
                          on_click=AlternativeDisplayState.update_display_info),
            ),
        ),
    )
